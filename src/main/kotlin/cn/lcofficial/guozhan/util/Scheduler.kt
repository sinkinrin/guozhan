...
fun runLater(delay: Long, task: (ScheduledTask) -> Unit): ScheduledTask =
    plugin.server.globalRegionScheduler.runDelayed(plugin, task, delay / 50)

fun runRepeat(delay: Long, period: Long, task: (ScheduledTask) -> Unit): ScheduledTask =
    plugin.server.globalRegionScheduler.runAtFixedRate(plugin, task, delay / 50, period / 50)
...
fun Entity.runLater(delay: Long, retired: () -> Unit = {}, task: (ScheduledTask) -> Unit): ScheduledTask? =
    scheduler.runDelayed(plugin, task, { retired() }, delay / 50)
...
fun Entity.runRepeat(
    ...
    scheduler.runAtFixedRate(plugin, task, { retired() }, delay / 50, period / 50)

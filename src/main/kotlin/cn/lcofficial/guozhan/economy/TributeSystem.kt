...
object TributeSystem {
    ...
    fun initialize() {
        ...
        // 设置定时任务，每小时处理一次自动贡献 - 使用Folia主线程调度器
        // 🔧 v1.3.23: 修复数据竞争问题 - 在主线程中执行以避免并发修改Country对象
        autoTributeTask?.cancel()
        autoTributeTask = cn.lcofficial.guozhan.util.runRepeat(
            delay = 20L * 60L * 10L, // 10分钟后开始(ticks)
            period = 20L * 60L * 60L  // 每小时执行一次(ticks)
        ) { _ ->
            try {
    ...
    fun establishTributeRelation(tributeCountry: Country, receivingCountry: Country, tributeRate: Int): Boolean {
        ...
        // 广播贡献关系建立消息
        val message = "§6${tributeCountry.name} §e向§6${receivingCountry.name} §e建立了贡献关系，贡献比率为§6${tributeRate}%§e！"
     ...
     * 🔧 v1.3.23: 修复数据竞争问题 - 此方法现在应该在主线程中调用
     * @return 处理的贡献关系数量
     */
    ...
    private fun notifyCountryMembers(tributeCountry: Country, receivingCountry: Country, goldAmount: Int, diamondAmount: Int) {
        ...
        // 记录到日志
        Guozhan.instance.logger.info("${tributeCountry.name}向${receivingCountry.name}贡献了${goldAmount}金币，${diamondAmount}钻石，总价值${totalValue}，当前关系${relation?.relationType}")
    }
}
